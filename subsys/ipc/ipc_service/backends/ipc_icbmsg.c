 ...
 * --------------
 *
 * ICMsg is used to send and receive small 3-byte messages.
 *
 *  - Send data
			...
			enum msg_type msg_type, uint8_t ept_addr)
{
	...
	return icmsg_send_wrapper(dev_data, msg_type, ept_addr, rx_block_index);
}
		      ...
		      uint8_t ept_addr, size_t tx_block_index, size_t size)
{
	...
	r = icmsg_send_wrapper(dev_data, msg_type, ept_addr, tx_block_index);
	if (r < 0) {
...
static void ept_bound_process(struct k_work *item)
{
	...
	if (!(atomic_get(&dev_data->flags) & FLAG_ICMSG_BOUNDED)) {
		return;
	}
				 ...
				 uint8_t ept_addr)
{
	...
	/* When releasing bound message, validate RX state of the endpoint. */
...
static void received(const void *data, size_t len, void *priv)
{
	...
	const struct icmsg_message *message = (const struct icmsg_message *)data;
	uint8_t ept_addr;
	int r = 0;
	...
	case MSG_RELEASE_BOUND:
		r = received_release_data(dev_data, message->block_index, ept_addr);
		break;
	case MSG_BOUND:
...
static int open(const struct device *instance)
{
	...
	LOG_DBG("  ICMsg, TX %d at 0x%08X, RX %d at 0x%08X",
		...
		(uint32_t)conf->icmsg_config.rx_shm_size,
		(uint32_t)conf->icmsg_config.rx_shm_addr);
...
/* Just make sure that cache is power of 2, because only those we are supporting. */
BUILD_ASSERT(IS_POWER_OF_TWO(BLOCK_ALIGNMENT),
...
#define GET_MEM_ADDR_INST(i, direction) \
	ROUND_UP(DT_REG_ADDR(DT_INST_PHANDLE(i, direction##_region)), BLOCK_ALIGNMENT)
...
#define GET_MEM_END_INST(i, direction)							\
	ROUND_DOWN(DT_REG_ADDR(DT_INST_PHANDLE(i, direction##_region)) +		\
		   DT_REG_SIZE(DT_INST_PHANDLE(i, direction##_region)), BLOCK_ALIGNMENT)
...
#define DEFINE_BACKEND_DEVICE(i)							\
	...
	{										\
		.icmsg_config = {							\
			...
			.tx_shm_addr = GET_MEM_ADDR_INST(i, tx),			\
			.rx_shm_size = GET_ICMSG_SIZE_INST(i, rx, tx),			\
			.rx_shm_addr = GET_MEM_ADDR_INST(i, rx),			\
